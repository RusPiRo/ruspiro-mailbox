...
impl Mailbox {
    ...
    pub fn get_arm_memory(&self) -> MailboxResult<(u32, u32)> {
        let mut tag = propertytags::ArmMemoryGet::new();
        send_message(MailboxChannel::PropertyTagsVc, &mut tag).and_then(|tag| {
    ...
    /// # use ruspiro_mailbox::*;
    /// # fn demo() {
    /// let clock_rate = MAILBOX
    ///     .take_for(|mb| mb.get_clockrate(ArmClockId::Core)
    ///     .expect("unable to get core clock rate"));
    /// # }
    /// ```
    pub fn get_clockrate(&self, clock_id: ArmClockId) -> MailboxResult<u32> {
        ...
        send_message(MailboxChannel::PropertyTagsVc, &mut tag)
            .and_then(|clock_rate_get| Ok(clock_rate_get.get_response().clock_rate))
    }
    ...
    /// # use ruspiro_mailbox::*;
    /// # fn demo() {
    /// let new_clock_rate = MAILBOX
    ///     .take_for(|mb| mb.set_clockrate(ArmClockId::Core, 250_000_000)
    ///     .expect("unable to set core clock rate"));
    /// # }
    /// ```
    pub fn set_clockrate(&self, clock_id: ArmClockId, rate: u32) -> MailboxResult<u32> {
        let mut tag = propertytags::ClockrateSet::new(clock_id as u32, rate, 0x0);
        send_message(MailboxChannel::PropertyTagsVc, &mut tag)
            .and_then(|clock_rate_set| Ok(clock_rate_set.get_response().clock_rate))
    }
}
